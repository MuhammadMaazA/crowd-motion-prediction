"""
Training script for Trajectory Transformer on ETH/UCY
======================================================
...
python train_trajectory_transformer.py --holdout eth --epochs 200
"""
...
def train(holdout, epochs=200, batch_size=64, lr=1e-3, d_model=128, nhead=4,
          num_enc=2, num_dec=2, dim_ff=128, max_neighbours=5,
    ...
    model = TrajectoryTransformer(
        ...
        dim_ff=dim_ff, max_nb=max_neighbours,
    ).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_every == 0 or epoch == epochs:
            ...
            if metrics["ade"] < best_ade:
                ...
                ckpt_path = os.path.join(ckpt_dir, f"{ft_prefix}transformer_{holdout}.pt")
                torch.save({
    ...
    axes[0].set_title(f"Transformer Loss ({holdout} held out)")
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(ckpt_dir, f"transformer_{holdout}_curve.png"), dpi=150)
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    train(holdout=args.holdout, epochs=args.epochs, batch_size=args.batch_size,
          ...
          num_enc=args.num_enc, num_dec=args.num_dec, dim_ff=args.dim_ff,
